...
// @namespace   https://github.com/sndv/aoc-userscripts
...
const MAIN_ARTICLE_SELECTOR = "main > article"
...
function addCurrentLeaderboardLabel(leaderboardId) {
  if (leaderboardId in labelMap) {
    const articleElement = document.querySelector(MAIN_ARTICLE_SELECTOR);
    const leaderboardLabel = document.createElement("span");
    leaderboardLabel.textContent = `(${labelMap[leaderboardId]})`
    leaderboardLabel.style.color = "dimgray";
    articleElement.insertBefore(leaderboardLabel, articleElement.firstChild);
  } else {
    console.log(`${LOG_PREFIX}No label for leaderboard ${leaderboardId}`);
  }
}
...
function handleSingleLeaderboardPage(url) {
  const match = url.match(/\/private\/view\/(\d+)\/?$/);
  if (match) {
    readLabelMap();
    addCurrentLeaderboardLabel(match[1]);
  } else {
    console.log(`${LOG_PREFIX}Failed to parse private leaderboard id from url: ${url}`);
  }
}

const url = document.URL;
if (url.includes("view")) {
  handleSingleLeaderboardPage(url);
} else {
  handleListPage();
}
